...
agent_waiting_neightbours(Name, Neightbours) ->
    receive
        ...
        setup_end -> io:format("Agent ~p has ~p neightbours~n", [Name, length(Neightbours)]);
            % call sthg like agent_loop
        Any -> 
            io:format("Agent ~p receives and ignores message '~p' while waiting for neightbours~n"
                        , [Name, Any])
    end.
